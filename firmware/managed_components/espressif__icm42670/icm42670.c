...
esp_err_t icm42670_create(i2c_master_bus_handle_t bus, const uint8_t dev_addr, icm42670_handle_t *out_handle)
{
    ...
    esp_err_t ret = i2c_master_bus_add_device(bus, &dev_cfg, &sensor->dev);
    if (ret != ESP_OK) { free(sensor); return ret; }
    ...
    *out_handle = (icm42670_handle_t) sensor;
    return ESP_OK;
}
...
static esp_err_t icm42670_write(icm42670_handle_t sensor, const uint8_t reg_start_addr, const uint8_t *data_buf, const uint8_t data_len)
{
    ...
    return i2c_master_transmit(sens->dev, buf, data_len + 1, 1000);
}
...
static esp_err_t icm42670_read(icm42670_handle_t sensor, const uint8_t reg_start_addr, uint8_t *data_buf, const uint8_t data_len)
{
    ...
    uint8_t reg = reg_start_addr;
    return i2c_master_transmit_receive(sens->dev, &reg, 1, data_buf, data_len, 1000);
}
