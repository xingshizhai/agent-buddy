...
/* button v4.x custom driver for the touch-based MAIN button */
typedef struct {
    button_driver_t base;
} bsp_touch_btn_driver_t;

static uint8_t bsp_touch_btn_get_level(button_driver_t *drv)
{
    assert(tp);
#if (CONFIG_ESP_LCD_TOUCH_MAX_BUTTONS > 0)
    uint8_t home_btn_val = 0x00;
    esp_lcd_touch_get_button_state(tp, 0, &home_btn_val);
    return home_btn_val ? 1 : 0;
#else
    return 0;
#endif
}

static bsp_touch_btn_driver_t bsp_main_btn_driver = {
    .base = {
        .get_key_level = bsp_touch_btn_get_level,
    },
};
...
esp_err_t bsp_i2c_init(void)
{
    ...
    BSP_ERROR_CHECK_RETURN_ERR(i2c_new_master_bus(&bus_cfg, &s_i2c_bus_handle));
...
static esp_err_t bsp_i2c_device_probe(uint8_t addr)
{
    ...
    if (i2c_master_probe(s_i2c_bus_handle, addr, 10) == ESP_OK) {
        ret = ESP_OK;
    }
    return ret;
}
...
esp_err_t bsp_iot_button_create(button_handle_t btn_array[], int *btn_cnt, int btn_array_size)
{
    ...
    /* CONFIG button — GPIO_NUM_0, active-low */
    button_config_t btn_cfg = {0};
    button_gpio_config_t gpio_cfg = {
        .gpio_num = BSP_BUTTON_CONFIG_IO,
        .active_level = 0,
    };
    ret = iot_button_new_gpio_device(&btn_cfg, &gpio_cfg, &btn_array[BSP_BUTTON_CONFIG]);
    if (ret != ESP_OK) { return ret; }
    if (btn_cnt) { (*btn_cnt)++; }

    /* MUTE button — GPIO_NUM_1, active-low */
    gpio_cfg.gpio_num = BSP_BUTTON_MUTE_IO;
    ret = iot_button_new_gpio_device(&btn_cfg, &gpio_cfg, &btn_array[BSP_BUTTON_MUTE]);
    if (ret != ESP_OK) { return ret; }
    if (btn_cnt) { (*btn_cnt)++; }

    /* MAIN button — touch circle via GT911, custom driver */
    if (tp == NULL) {
        BSP_ERROR_CHECK_RETURN_ERR(bsp_touch_new(NULL, &tp));
    }
    ret = iot_button_create(&btn_cfg, &bsp_main_btn_driver.base, &btn_array[BSP_BUTTON_MAIN]);
    if (ret != ESP_OK) { return ret; }
    if (btn_cnt) { (*btn_cnt)++; }
